...
const fontSrc = path.join(root, "public", "fonts");
...
let ev = {
  ...
  officialIso: "2026-09-12T15:00:00+01:00",
  endIso: "2026-09-12T19:45:00+01:00",
};
...
function home() {
  const homeStats = proofStats.slice(0, 4).map(([value, label]) => `<article aria-label="${esc(`${value} ${label}`)}"><strong>${value}</strong><span>${label}</span></article>`).join("");
  const homeDestinations = [
    ["fr", "France", "FR", "Paris", "Construire une cohérence académique qui se défend.", "Choix de formation, logique du parcours, Campus France, entretien et procédure consulaire : chaque étape doit renforcer la précédente.", "Projet académique · Campus France · Visa"],
    ["ca", "Canada", "CA", "Montréal", "Aligner admission, province, budget et calendrier.", "Le projet canadien demande une lecture précise des coûts, de l'établissement, de la province et des délais. Nous construisons une feuille de route réaliste.", "Admission · Financement · Permis d'études"],
    ["de", "Allemagne", "DE", "Berlin", "Anticiper la langue, le financement et les étapes locales.", "Niveau linguistique, reconnaissance du parcours, financement et chronologie administrative doivent être étudiés dès la phase d'orientation.", "Langue · Financement · Calendrier"],
    ["es", "Espagne", "ES", "Madrid", "Donner de la lisibilité à une destination sous-estimée.", "Programme, langue d'enseignement, ressources et organisation consulaire : nous clarifions les règles avant le dépôt du dossier.", "Programme · Langue · Visa national"],
    ["ru", "Russie", "RU", "Moscou", "Sécuriser le parcours de l'admission jusqu'au départ.", "Établissement, invitation, budget et préparation au voyage sont organisés dans le bon ordre pour éviter les décisions tardives.", "Admission · Invitation · Préparation au départ"],
  ];
  const destinationTabs = homeDestinations.map(([key, name, code, city, title, copy, focus], index) => `<button class="cfh-destination-tab${index === 0 ? " is-active" : ""}" type="button" role="tab" aria-selected="${index === 0 ? "true" : "false"}" data-home-destination="${key}" data-label="Destination ${String(index + 1).padStart(2, "0")} · ${esc(name)}" data-title="${esc(title)}" data-copy="${esc(copy)}" data-code="${code}" data-city="${esc(city)}" data-focus="${esc(focus)}"><img src="/images/flags/${key}.svg" alt="" />${name}<span>${String(index + 1).padStart(2, "0")}</span></button>`).join("");
  ...
  return page({
    ...
    body: `<section class="cfh-event-bar" aria-label="Annonce SGVE 2026"><p><strong>SGVE 2026</strong><span>${ev.date} · ${ev.place}</span></p><a href="/sgve-2026/">Découvrir la conférence <span aria-hidden="true">↗</span></a></section>
    <section class="cfh-hero"><div class="cfh-grid" aria-hidden="true"></div><div class="cfh-orbit cfh-orbit-one" aria-hidden="true"></div><div class="cfh-orbit cfh-orbit-two" aria-hidden="true"></div><div class="cfh-hero-copy"><p class="cfh-overline"><span>Stratégie de mobilité internationale</span><em>Cameroun → Monde</em></p><h1>Votre projet mérite mieux qu'un dossier. <span>Il mérite une direction.</span></h1><p class="cfh-lead">Nous transformons les ambitions d'études et de mobilité en parcours clairs, cohérents et défendables — de la première orientation jusqu'à l'installation.</p><div class="cfh-actions"><a class="cfh-button cfh-button-primary" href="${site.whatsappFr}" target="_blank" rel="noreferrer">Faire diagnostiquer mon projet <span aria-hidden="true">↗</span></a><a class="cfh-text-action" href="#expertises"><i aria-hidden="true">↓</i> Explorer nos expertises</a></div><div class="cfh-assurance"><span aria-hidden="true">CF</span><p><strong>Accompagnement stratégique.</strong> Aucun visa n'est garanti : chaque décision appartient aux institutions compétentes.</p></div></div><div class="cfh-hero-visual"><figure class="cfh-hero-photo"><img src="${visuals.advisory}" alt="Une conseillère CF Consulting Travel échange avec une étudiante et son parent" /><figcaption>Diagnostic · Cohérence · Stratégie</figcaption></figure><div class="cfh-route-card"><p>Votre trajectoire</p><div><span>Douala</span><i></i><b aria-hidden="true">✦</b><i></i><span>Votre avenir</span></div><small>Une feuille de route adaptée à votre profil.</small></div><div class="cfh-seal" aria-hidden="true"><span>CF</span><p>Votre expert en<br />mobilité internationale</p></div></div></section>
    <section class="cfh-impact" aria-label="Chiffres clés CF Consulting Travel">${homeStats}<p>Une expérience de terrain au service de chaque nouveau parcours.</p></section>
    <section class="cfh-manifesto"><div class="cfh-marker"><span>02</span><p>Notre conviction</p></div><div><p class="cfh-eyebrow">Bien plus qu'un dossier</p><h2>Un projet international ne se remplit pas. <em>Il se construit.</em></h2></div><div class="cfh-manifesto-copy"><p>Un bon dossier raconte une trajectoire crédible : votre profil, votre formation, vos ressources, votre destination et votre projet professionnel doivent parler d'une seule voix.</p><a class="cfh-arrow-link" href="#methode">Comprendre notre approche <span aria-hidden="true">→</span></a></div></section>
    <section class="cfh-expertises" id="expertises"><header class="cfh-section-heading"><div><p class="cfh-eyebrow">Nos expertises</p><h2>Un accompagnement pensé comme une <span>architecture de solutions.</span></h2></div><p>Chaque intervention répond à une étape précise de votre parcours. Choisissez votre situation ; nous construisons la stratégie avec vous.</p></header><div class="cfh-service-mosaic"><article class="cfh-service cfh-service-image"><img src="${serviceVisuals["/orientation-etudes-etranger/"][0]}" alt="Conseiller présentant une stratégie d'orientation internationale" loading="lazy" /><div class="cfh-service-shade"></div><div><span>01 / Orientation</span><h3>Choisir la bonne direction avant de choisir une école.</h3><p>Profil, projet, budget, pays et perspectives : nous alignons les décisions avant les démarches.</p><a href="/orientation-etudes-etranger/">Découvrir <b aria-hidden="true">↗</b></a></div></article><article class="cfh-service cfh-service-orange"><span>02 / Études</span><i aria-hidden="true">CF</i><h3>Campus France, de A à Z.</h3><p>Projet d'études, choix des formations, dossier, entretien et suivi jusqu'à l'accord préalable.</p><a href="/accompagnement-campus-france/">Voir l'accompagnement <b aria-hidden="true">↗</b></a></article><article class="cfh-service cfh-service-cream"><span>03 / Visa</span><h3>Transformer des pièces en preuves cohérentes.</h3><p>Financement, logement, garanties de retour et logique du projet : chaque élément trouve sa juste place.</p><a href="/visa-etudiant/">Préparer mon visa <b aria-hidden="true">↗</b></a></article><article class="cfh-service cfh-service-dark"><span>04 / Refus & recours</span><p class="cfh-service-kicker">Votre projet n'a pas encore dit son dernier mot.</p><h3>Comprendre le refus. Choisir la réponse adaptée.</h3><p>Analyse du motif, recours gracieux ou orientation vers le contentieux au tribunal administratif de Nantes.</p><a href="/recours-visa/">Analyser mon refus <b aria-hidden="true">↗</b></a></article><article class="cfh-service cfh-service-outline"><span>05 / Installation</span><strong aria-hidden="true">360°</strong><h3>Le visa n'est pas la fin du parcours.</h3><p>Accueil, logement, CAF, CPAM, OFII, alternance, renouvellement et changement de statut.</p><a href="/contact/">Préparer mon arrivée <b aria-hidden="true">↗</b></a></article></div></section>
    <section class="cfh-method" id="methode"><div class="cfh-method-intro"><p class="cfh-eyebrow">La méthode CF</p><h2>Une stratégie lisible.<br /><span>Quatre décisions clés.</span></h2><p>Nous ne commençons pas par remplir des formulaires. Nous commençons par comprendre ce qui rend votre projet crédible.</p></div><div class="cfh-method-board"><div class="cfh-method-tabs" role="tablist" aria-label="Étapes de la méthode CF"><button class="is-active" type="button" role="tab" aria-selected="true" data-home-method="0"><span>01</span>Diagnostiquer</button><button type="button" role="tab" aria-selected="false" data-home-method="1"><span>02</span>Orienter</button><button type="button" role="tab" aria-selected="false" data-home-method="2"><span>03</span>Structurer</button><button type="button" role="tab" aria-selected="false" data-home-method="3"><span>04</span>Préparer</button></div><div class="cfh-method-stage" aria-live="polite"><span data-home-method-step>Étape 01</span><h3 data-home-method-title>Voir clair avant d'agir.</h3><p data-home-method-copy>Nous analysons le parcours, les objectifs, les contraintes, les ressources et les fragilités avant de recommander une procédure.</p><ul data-home-method-list><li>Lecture du profil et du parcours</li><li>Identification des zones de risque</li><li>Priorités et calendrier d'action</li></ul><div class="cfh-radar" aria-hidden="true"><i></i><i></i><i></i><b>CF</b></div></div></div></section>
    <section class="cfh-destinations" id="destinations"><header class="cfh-section-heading"><div><p class="cfh-eyebrow">Cinq destinations</p><h2>Un cap commun.<br /><span>Cinq règles du jeu.</span></h2></div><p>Les mêmes documents ne racontent pas la même histoire partout. Nous adaptons la stratégie aux exigences de chaque destination.</p></header><div class="cfh-destination-explorer"><div class="cfh-destination-tabs" role="tablist" aria-label="Destinations accompagnées">${destinationTabs}</div><div class="cfh-destination-stage" aria-live="polite"><div><p data-home-destination-label>Destination 01 · France</p><h3 data-home-destination-title>Construire une cohérence académique qui se défend.</h3><p data-home-destination-copy>Choix de formation, logique du parcours, Campus France, entretien et procédure consulaire : chaque étape doit renforcer la précédente.</p><a class="cfh-arrow-link" href="/services/">Explorer nos accompagnements <span aria-hidden="true">→</span></a></div><div class="cfh-destination-visual" aria-hidden="true"><div class="cfh-globe"><i></i><i></i><i></i><span data-home-destination-code>FR</span></div><div class="cfh-flight"><span>Cameroun</span><i></i><b>✦</b><i></i><span data-home-destination-city>Paris</span></div><p data-home-destination-focus>Projet académique · Campus France · Visa</p></div></div></div></section>
    <section class="cfh-sgve"><img src="${visuals.sgveHero}" alt="Salle de conférence de la SGVE 2026 à Douala" loading="lazy" /><div class="cfh-sgve-overlay"></div><div class="cfh-sgve-content"><p class="cfh-sgve-brand">SG<span>VE</span><small>2026</small></p><p class="cfh-eyebrow">La conférence qui dit la vérité sur le visa étudiant</p><h2>Une journée pour éviter des mois d'erreurs.</h2><p>Étudiants, parents et financeurs : rencontrez celles et ceux qui connaissent les vraies règles du parcours international.</p><div class="cfh-sgve-facts"><article><span>Quand</span><strong>${ev.date}</strong></article><article><span>Où</span><strong>${ev.place}</strong></article><article><span>Horaires</span><strong>Portes ${ev.time} · Début ${ev.officialTime} · Fin ${ev.endTime}</strong></article></div><a class="cfh-button cfh-button-primary" href="/sgve-2026/">Découvrir la SGVE 2026 <span aria-hidden="true">↗</span></a></div></section>
    <section class="cfh-proof"><div class="cfh-quote"><p class="cfh-eyebrow">Témoignages et preuves sociales</p><span aria-hidden="true">“</span><blockquote>Notre rôle n'est pas de vous vendre un rêve. Il est de vous aider à construire un projet qui résiste aux questions.</blockquote><p>La direction · CF Consulting Travel</p></div><div class="cfh-proof-grid"><article><span>5</span><p>destinations accompagnées depuis le Cameroun</p></article><article><span>72 h</span><p>pour recevoir une première analyse personnalisée</p></article><article><span>2</span><p>bureaux de proximité entre Massy et Douala</p></article><article><span>1</span><p>équipe autour de votre parcours, pas seulement de vos documents</p></article></div></section>
    <section class="cfh-contact"><div><p class="cfh-eyebrow">Votre prochaine décision commence ici</p><h2>Parlons de votre projet avant que vous ne déposiez votre dossier.</h2><p>Un premier échange permet d'identifier la bonne procédure, les priorités et les points qui méritent d'être renforcés.</p></div><div><a class="cfh-button cfh-button-light" href="${site.whatsappFr}" target="_blank" rel="noreferrer">Je veux être accompagné <span aria-hidden="true">↗</span></a><a class="cfh-button cfh-button-ghost" href="https://calendly.com/cfconsultingtravel/consultation-cf-consulting" target="_blank" rel="noreferrer">Prendre rendez-vous</a><small>Consultation personnalisée · réponse sous 72 h</small></div></section>`,
  });
}
...
function sgve() {
  ...
  const programme = [
    ["14h00 – 14h50", "Accueil et cérémonie d'ouverture", "Ouverture des portes, accueil, contrôle des inscriptions et installation progressive."],
    ["14h50 – 15h00", "Accueil et cérémonie d'ouverture", "Installation finale et derniers contrôles techniques."],
    ["15h00 – 15h03", "Accueil et cérémonie d'ouverture", "Hymne national du Cameroun."],
    ["15h03 – 15h15", "Accueil et cérémonie d'ouverture", "Mini-show d'ouverture."],
    ["15h15 – 15h20", "Accueil et cérémonie d'ouverture", "Discours institutionnel."],
    ["15h20 – 15h22", "Accueil et cérémonie d'ouverture", "Lancement officiel de la conférence."],
    ["15h22 – 15h42", "Panel 1 — Partir où, mais pour quel projet ?", "Thème 1 : Comment construire une orientation académique cohérente ?"],
    ["15h42 – 16h02", "Panel 1 — Partir où, mais pour quel projet ?", "Thème 2 : Après les études, quel avenir ?"],
    ["16h02 – 16h32", "Panel 2 — Cinq destinations, cinq règles du jeu", "Thème 1 : Maîtrisez la feuille de route infaillible pour décrocher votre visa vers l'Allemagne et l'Espagne."],
    ["16h32 – 17h02", "Panel 2 — Cinq destinations, cinq règles du jeu", "Thème 2 : Maîtrisez la feuille de route infaillible pour décrocher votre visa vers le Canada."],
    ["17h02 – 17h22", "Panel 2 — Cinq destinations, cinq règles du jeu", "Thème 3 : Maîtrisez la feuille de route infaillible pour décrocher votre visa vers la Russie."],
    ["17h22 – 17h42", "Panel 2 — Cinq destinations, cinq règles du jeu", "Thème 4 : Maîtrisez la feuille de route infaillible pour décrocher votre visa vers la France."],
    ["17h42 – 18h02", "Intervention spéciale", "Thème : Devenir une solution, pas seulement un diplômé."],
    ["18h02 – 18h22", "Panel 3 — Trois clés stratégiques pour décrocher votre visa", "Thème 1 : AVI et assurance — Pourquoi faut-il les anticiper avant la demande de visa ?"],
    ["18h22 – 18h32", "Panel 3 — Trois clés stratégiques pour décrocher votre visa", "Thème 2 : Logement étudiant — Pourquoi attendre le visa pour commencer ses recherches peut-il déjà être trop tard ?"],
    ["18h32 – 18h47", "Panel 4 — Visa refusé : votre projet n'a pas encore dit son dernier mot", "Les solutions et les démarches à envisager après un refus de visa."],
    ["18h47 – 19h02", "Panel 5 — Visa en poche : bienvenue dans votre nouvelle vie", "Les premières étapes à anticiper après l'obtention du visa."],
    ["19h02 – 19h32", "Échanges et clôture", "Questions-réponses avec le public."],
    ["19h32 – 19h37", "Échanges et clôture", "Remise des chèques et des prix."],
    ["19h37 – 19h40", "Échanges et clôture", "Mot officiel de clôture."],
    ["19h40 – 19h43", "Échanges et clôture", "Photo officielle de clôture."],
    ["19h43 – 19h45", "Échanges et clôture", "Remerciements et fin de la conférence."],
  ];
  return page({
    title: `${ev.title} — ${ev.long} à Douala`,
    desc: `Participez gratuitement à ${ev.title}, conférence organisée par ${site.name} le ${ev.date} au Krystal Palace de Douala. Ouverture des portes à ${ev.time}, début officiel à ${ev.officialTime} et fin à ${ev.endTime}.`,
    route: "/sgve-2026/",
    kind: "event",
...
async function build() {
  ...
  if (existsSync(fontSrc)) await copyDir(fontSrc, fontOut);
