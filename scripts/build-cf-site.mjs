...
function header() {
  return `<header class="top"><div class="nav-shell"><a class="brand" href="/" aria-label="Accueil CF Consulting Travel"><img src="/images/sgve/logo-cf-consulting.png" alt="Logo CF Consulting Travel" /><span><strong>${site.name}</strong><small>Mobilite internationale</small></span></a><button class="menu-btn" data-menu-button type="button" aria-label="Ouvrir le menu principal" aria-expanded="false" aria-controls="navigation-principale">Menu</button><nav id="navigation-principale" aria-label="Navigation principale" data-menu>${navLinks.map(([label, href]) => `<a href="${href}">${label}</a>`).join("")}<a class="nav-cta" href="${site.whatsappFr}" target="_blank" rel="noreferrer" aria-label="Nous joindre sur WhatsApp">Nous joindre sur WhatsApp</a></nav></div></header>`;
}

function footer() {
  return `<footer id="contact"><div><a class="brand" href="/"><img src="/images/sgve/logo-cf-consulting.png" alt="Logo CF Consulting Travel" /><span><strong>${site.name}</strong><small>Mobilite internationale</small></span></a><p>Aucun resultat de visa n'est garanti : chaque dossier depend des criteres des institutions competentes.</p><div class="actions"><a class="btn primary" href="${site.whatsappFr}" target="_blank" rel="noreferrer">Nous joindre sur WhatsApp</a></div></div><div><h2>Services</h2>${serviceLinks.slice(0, 3).map(([label, href]) => `<a href="${href}">${label}</a>`).join("")}<a href="/services/">Tous les services</a><a href="/temoignages/">Temoignages</a><a href="/blog/">Blog / conseils</a></div><div><h2>Contacts</h2><a href="mailto:${site.email}">Email principal : ${site.email}</a><a href="mailto:${site.fallbackEmail}">Email secondaire : ${site.fallbackEmail}</a><a href="tel:+33656737225">Téléphone France : ${site.phoneFr}</a><a href="${site.whatsappFr}" target="_blank" rel="noreferrer">Contact WhatsApp : ${site.whatsappPhone}</a><a href="tel:+237657605017">Téléphone Cameroun : ${site.phoneCm}</a><p>Adresse France : ${site.address}</p></div><div><h2>Cadre legal</h2><a href="/mentions-legales/">Mentions legales</a><a href="/politique-confidentialite/">Politique de confidentialite</a><a href="/conditions-utilisation/">Conditions d'utilisation</a><a href="/donnees-inscriptions-sgve-2026/">Donnees inscriptions SGVE 2026</a><a href="${site.channel}" target="_blank" rel="noreferrer">Chaîne WhatsApp SGVE 2026</a></div><p class="copyright">© 2026 CF Consulting Travel. Tous droits réservés.</p></footer>`;
}
...
function home() {
  const homeBody = `<section class="home-hero"><div class="home-hero-copy"><h1>Votre projet d'études à l'étranger mérite une stratégie claire.</h1><p class="lead">CF Consulting Travel accompagne les étudiants, les parents et les porteurs de projets avec une méthode sobre : clarifier le parcours, structurer le dossier, préparer les étapes clés et avancer sans promesse irréaliste.</p><div class="actions"><a class="btn primary" href="${site.whatsappFr}" target="_blank" rel="noreferrer">Parler à un conseiller</a><a class="btn secondary" href="/sgve-2026/">Découvrir SGVE 2026</a></div><div class="hero-proof" aria-label="Indicateurs de confiance"><p><strong>+1000</strong><span>étudiants orientés ou formés</span></p><p><strong>5</strong><span>destinations principales</span></p><p><strong>+25</strong><span>écoles partenaires</span></p></div></div><figure class="home-editorial" aria-label="Composition visuelle premium autour d'un dossier de mobilité internationale"><div class="editorial-panel"><div class="editorial-topline"><span>CF Mobility Desk</span><b>2026</b></div><div class="editorial-doc"><span>Diagnostic stratégique</span><strong>Projet d'études</strong><p>Profil, parcours, preuves et calendrier.</p><i></i><i></i><i></i></div><div class="editorial-passport"><em></em><strong>Visa étudiant</strong><span>Dossier structuré</span></div><div class="editorial-map"><span>FR</span><span>CA</span><span>ES</span><span>RU</span><span>DE</span></div><div class="editorial-advice"><small>Entretien conseil</small><strong>Feuille de route claire</strong></div></div></figure></section><section class="home-proof-section"><div><p class="section-label">Témoignages et preuves sociales</p><h2>Un accompagnement pensé pour rassurer, structurer et décider avec méthode.</h2><p>Le site CF Consulting Travel doit d'abord inspirer la confiance d'un cabinet de conseil : méthode, transparence, expertise et prudence sur les résultats.</p><div class="actions"><a class="btn primary" href="/contact/">Je veux être accompagné</a></div></div><div class="home-proof-grid">${proofStats.slice(0, 4).map(([value, label, text]) => `<article aria-label="${value} ${label}"><strong>${value}</strong><h3>${label}</h3><p>${text}</p></article>`).join("")}</div></section><section class="section" id="services"><p class="eyebrow">Services principaux</p><h2>Un site clair pour chaque besoin : etudes, voyage, recours et conseil.</h2><div class="grid four">${serviceLinks.map(([title, href, text]) => serviceCard(title, href, text)).join("")}</div></section><section class="section split visual-split" id="methode"><div><p class="eyebrow">Methode CF</p><h2>Diagnostiquer, structurer, preparer.</h2><p>CF Consulting Travel aide chaque candidat a mieux comprendre son profil, ses objectifs, ses justificatifs et les limites de son dossier avant toute demarche importante.</p><div class="actions"><a class="btn primary" href="/contact/">Contacter l'equipe</a><a class="btn secondary" href="/a-propos/">A propos de CF</a></div></div><ol class="timeline"><li><strong>Diagnostic</strong><span>Analyse du profil, de la destination et du besoin reel.</span></li><li><strong>Feuille de route</strong><span>Priorites, documents, calendrier et points de vigilance.</span></li><li><strong>Preparation</strong><span>Conseils, relecture et accompagnement jusqu'aux etapes cles.</span></li></ol></section><section class="banner event-strip"><div><p class="eyebrow">Evenement officiel</p><h2>SGVE 2026 - ${ev.long}</h2><p>${ev.date} a ${ev.time} - ${ev.place}. Conference gratuite sur inscription.</p></div><img src="${visuals.conference}" alt="" aria-hidden="true" loading="lazy" /><div class="actions"><a class="btn primary" href="/sgve-2026/">Réserver ma place à SGVE 2026</a><a class="btn secondary light" href="${site.whatsappFr}" target="_blank" rel="noreferrer">Nous joindre sur WhatsApp</a></div></section>`;
  return page({
...
async function build() {
  ...
  await writeFile(path.join(out, "styles.css"), finalCss + premiumPolishCss + premiumRedesignV2Css + premiumHeroEditorialCss, "utf8");
